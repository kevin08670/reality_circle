...
struct usermgr * usermgr_create() {
	...
	if (NULL != fp) {
		...
		trace("usermgr local acc.txt fail.");
	}
...
bool user_local_is_user_exist(struct usermgr *mgr, const char *key) {
...
void print_online_user(struct usermgr *mgr) {
	...
	while ((element = it->next(it))) {
		...
		trace("%s %"PRIu64"  %s %s", key, user->user_id, str_user_name, str_user_cipher);
	}
	cstl_map_delete_iterator(it);
}
