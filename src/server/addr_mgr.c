...
void print_domain_address(struct ip_addr_cache *addr_cache) {
	struct cstl_iterator* it = cstl_map_new_iterator(addr_cache->resolved_ips);
	const void* element;
	while ((element = it->next(it))) {
		const char ** key = (const char**)it->current_key(it);
		const struct address_timestamp **addr = (const struct address_timestamp **)it->current_value(it);
		xtrace(text_color_blue, "print domain:%s ip:%s", *key, inet_ntoa( (*addr)->address.addr4.sin_addr) );
	}
