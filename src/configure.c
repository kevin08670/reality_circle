...
struct configure * configure_create(int argc, char * argv[]) {
	...
	int c = 0;
	char stritem[256] = { 0 };
	while ((c = getopt(argc, argv, "a:u:n:i:s:p:f:")) != -1) {
		switch (c) {
		case 'a':
		...
		case 'p':
			...
			break;
		case 'f':
			strncpy(cfg->fake_sni, optarg, 256);
			break;
		default:
			break;
		}
...
void string_to_uuid( char * data, uint8_t uuid[16] ) {
	...
	for ( size_t i = 0; i < sav_len; i++ ) {
		uint8_t hb = char_to_uint8(uuid_data[i *2 ]);
		uint8_t hl  = char_to_uint8(uuid_data[i *2 + 1]);
		uuid[i] = (hb << 4) | hl;
	}
}
...
//sessionIDУ�죬�鿴�Ƿ�Ϊinno�Խ�sessionID
