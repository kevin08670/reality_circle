...
int mbedtls_ssl_set_sessionid1(mbedtls_ssl_context *ssl, unsigned char sessionid[32])
{
    if (sessionid != NULL && ssl->session_negotiate) {
        ssl->session_negotiate->id_len = 32;
        memcpy(ssl->session_negotiate->id, sessionid, 32); 
    }
    return 0;
}
...
int mbedtls_ssl_set_sessionid(mbedtls_ssl_context *ssl, unsigned char sessionid[32])
{
    if (sessionid != NULL && ssl->session_negotiate) {
        ssl->session_negotiate->id_len = 32;
        memcpy(ssl->session_negotiate->id, sessionid, 32); 
...
// add by caozhenbiao  >>>
int mbedtls_ssl_get_client_random(mbedtls_ssl_context *ssl,unsigned char random[MBEDTLS_CLIENT_HELLO_RANDOM_LEN])
{
	if ( random != NULL && ssl->transform) {
		memcpy(random, ssl->transform->randbytes+MBEDTLS_SERVER_HELLO_RANDOM_LEN, 32);
	}
	return 0;
}
...
int mbedtls_ssl_get_client_sessionid(mbedtls_ssl_context *ssl, unsigned char sessionid[32])
{
	if (sessionid != NULL && ssl->session_negotiate) {
		memcpy(sessionid, ssl->session_negotiate->id, 32);
	}
	return 0;
}

int mbedtls_ssl_get_peer_sessionid(mbedtls_ssl_context *ssl, unsigned char sessionid[32])
{
	if (sessionid != NULL && ssl->session) {
		memcpy(sessionid, ssl->session->id, 32);
	}
	return 0;
}

const char *  mbedtls_ssl_get_client_sni(mbedtls_ssl_context *ssl){
	if ( ssl->handshake) {
		return (const char*)ssl->handshake->sni_name;
	}
	return NULL;
}
...
/*
*  add by caozhenbiao
 * Perform the SSL handshake step 1 clientHello
 */
int mbedtls_ssl_handshake_clienthello(mbedtls_ssl_context *ssl)
{
	int ret = 0;
	...
	/* Main handshake loop */
	while (ssl->state != MBEDTLS_SSL_SERVER_HELLO) {
		ret = mbedtls_ssl_handshake_step(ssl);
 ...
 * Perform the SSL handshake step 1 clientHello
 */
int mbedtls_ssl_handshake_serverhello_done(mbedtls_ssl_context *ssl)
{
	int ret = 0;
...
int mbedtls_ssl_handshake_workstatus(mbedtls_ssl_context *ssl)
{
	/* Sanity checks */
