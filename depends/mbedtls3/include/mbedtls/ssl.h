...
/*
	add by caozhenbiao
	用于获取handshake过程中每一个数据包clientHello 状态
*/
...
/**
	add by caozhenbiao
	用于客户端在发送ClientHello设置raandom
*/

int mbedtls_ssl_get_client_random(mbedtls_ssl_context *ssl, unsigned char random[32]);

/**
	add by caozhenbiao
	用于客户端在发送ClientHello设置sessionID
*/
int mbedtls_ssl_set_sessionid(mbedtls_ssl_context *ssl, unsigned char sessionid[32]);

/**
	add by caozhenbiao
	客户端获取客户端发送的ClientHello内 sessionID数据。
*/
